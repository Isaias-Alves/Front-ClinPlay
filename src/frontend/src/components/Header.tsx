...
export interface ClinicaHeaderData {
  id?: string; // Fallback
  clinicaId?: string; // Payload Real
  nome: string;
  cidade?: string; // Payload Real
  uf?: string; // Payload Real
}

interface HeaderProps {
  clinicas?: ClinicaHeaderData[];
...
const Header = ({
  ...
  // Identifica a clínica atual suportando tanto 'id' quanto 'clinicaId'
  const clinicaAtual =
    clinicas?.find((c) => (c.clinicaId || c.id) === clinicaSelecionadaId) ||
    clinicas?.[0];

  // Função auxiliar para formatar a localização
  const formatarLocalizacao = (cidade?: string, uf?: string) => {
    if (cidade && uf) return `${cidade} - ${uf}`;
    if (cidade) return cidade;
    if (uf) return uf;
    return "Vincule-se já!";
  };
        ...
        >
          ...
          <div className="">
            <h2 className="text-sm font-bold text-slate-800 leading-tight">
              {clinicaAtual?.nome || "Clínicas"}
            </h2>
            <p className="text-[11px] text-slate-400 font-medium">
              {formatarLocalizacao(clinicaAtual?.cidade, clinicaAtual?.uf)}
            </p>
          </div>

          <FiChevronDown
            className={`text-slate-400 ml-2 transition-transform duration-300 ${
              isClinicaMenuOpen ? "rotate-180" : ""
            }`}
          />
        </button>
              ...
              <div className="max-h-60 overflow-y-auto scrollbar-thin p-2 space-y-1">
                {clinicas?.length > 0 ? (
                  clinicas.map((clinica) => {
                    const idReal = clinica.clinicaId || clinica.id || "";
                    const isSelected = idReal === clinicaSelecionadaId;
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          if (isSelected) {
                            // Se já está selecionada, redireciona para a página de detalhes da clínica
                            navigate(`/clinicas/${idReal}`);
                          } else if (idReal) {
                            // Se não está selecionada, apenas muda o contexto ativo
                            onSelectClinica(idReal);
                          }
                          setIsClinicaMenuOpen(false);
                        }}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400">
                            {formatarLocalizacao(clinica.cidade, clinica.uf)}
                          </p>
                        </div>
                      </button>
                    );
